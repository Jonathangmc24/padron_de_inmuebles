  ...
  return (
    <SelectPrimitive.Root value={value || undefined} onValueChange={onChange}>
      <SelectPrimitive.Trigger
        className={`flex w-full items-center justify-between rounded-lg px-4 py-2.5 text-sm outline-none transition-colors ${
          esFormulario
            ? "text-white"
      ...
      <SelectPrimitive.Portal>
        <SelectPrimitive.Content
          ...
          sideOffset={6}
          className="z-50 overflow-hidden rounded-xl border border-gray-200 shadow-lg backdrop-blur-sm"
          style={{ backgroundColor: "#eea1be" }}
        >
          <SelectPrimitive.Viewport className="p-1.5">
            {opciones.map((op) => (
              <SelectPrimitive.Item
                key={op}
                value={op}
                className="relative mb-0.5 flex cursor-pointer select-none items-center rounded-lg px-3 py-2.5 pr-8 text-sm text-gray-700 outline-none transition-colors data-[highlighted]:bg-[#EEE4E7] data-[highlighted]:font-semibold data-[highlighted]:text-[#7B2645] data-[state=checked]:font-semibold data-[state=checked]:text-[#7B2645]"
              >
